        ...
        <React.Fragment>
            <div className="card border-dark mb-3" style={{ width: "18rem" }}>
                ...
                <div className="card-body d-flex flex-column justify-content-between">
                    ...
                    {item.properties.diameter && <p className="card-text"><strong>Diameter</strong> {item.properties.diameter}km</p>}
                    {/* This will get Vehicles properties */}
                    {item.properties.model && <p className="card-text"><strong>Model:</strong> {item.properties.model}</p>}
                    {item.properties.vehicle_class && <p className="card-text"><strong>Class:</strong> {item.properties.vehicle_class}</p>}
                    {item.properties.length && <p className="card-text"><strong>Length:</strong> {item.properties.length}mts</p>}
                    <Link to={`/${endpoint}/${item.uid}`} className="btn btn-primary">Learn more!</Link>
                </div>
            </div>
        </React.Fragment>
    );
